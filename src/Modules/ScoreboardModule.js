...
const Scoreboard = ({ lambdaOutput }) => {
  ...
  useEffect(() => {
    ...
    const data = lambdaOutput.map((array) => array.slice(0, -3));
    ...
    for (let i = 1; i <= data.length; i++) {
      ...
      if (i % timeFrame === 0 && i !== 0) {
        ...
        const revenue = lambdaOutput[i - 1].find(
        ...
        const modifiedRevenue = {
          ...
          values:
            timeFrame === "13"
              ? (revenue.values - previousRevenue).toFixed(1)
              : (revenue.values - previousRevenue).toFixed(0),
        };
  ...
  return (
    <div id="scoreboard-layout">
      <div id="scoreboard-top-layout">
        <h3 className="title"> Revenue Forecast </h3>
        <Select
